...
public class SendVerificationEmailUseCase {
    ...
    @Transactional
    public void execute(String email, String fullName, String verificationLink, int expiresInMinutes) {
        String subject = "Verify your Cartzilla email";
        String name = fullName == null || fullName.isBlank() ? "customer" : fullName;
        String body = "Hello " + name + ",\n\n"
                + "Use this link to verify your Cartzilla email. The link expires in "
                + expiresInMinutes + " minutes:\n" + verificationLink + "\n\n"
                + "If you did not create this account, you can ignore this email.";
        EmailLog emailLog = emailLogRepository.save(EmailLog.create(
                null, null, email, subject, "email_verification", "SMTP"));
        try {
