...
export function PaymentPage() {
  ...
  // Số tiền hiển thị lấy từ đơn thật trên server (server-authoritative),
  // không tin query param — tránh lệch với số tiền VNPay sẽ thu.
  ...
  const pay = async () => {
    ...
    try {
      const res = await paymentApi.createVnpay(
        orderId,
        order.totalAmount,
        `Thanh toan don hang ${shortCode(orderId)}`,
      );
      ...
      <div className="w-full rounded-lg border border-border bg-white p-8 text-center shadow-card">
        ...
        {orderLoading ? (
          ...
          <div className="mt-6 space-y-2 rounded-lg bg-page p-4 text-left text-sm">
            ...
            <div className="flex justify-between"><span className="text-ink-secondary">Số tiền</span><span className="font-headline text-lg font-bold tabular-nums text-brand">{order ? formatVnd(order.totalAmount) : '—'}</span></div>
          </div>
        )}
        {!orderLoading && order && !payable && (
          <div className="mt-4">
        ...
        <Button fullWidth size="lg" className="mt-6" loading={loading} disabled={orderLoading || !payable} onClick={pay}>
          {loading ? 'Đang chuyển tới cổng VNPay...' : 'Thanh toán ngay'}
        </Button>
        <button onClick={() => navigate(`/orders/${orderId}`)} className="mt-3 text-sm text-ink-muted hover:text-ink">
          Để sau — xem chi tiết đơn hàng
        </button>
